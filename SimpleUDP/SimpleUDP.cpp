...
namespace Net
{
    ...
    void InitNetwork()
    {
        ...
        if (result != NO_ERROR)
        {
            std::cerr << "WinSock2 " << "Failed to Initate WinSock2.2" << std::endl;;
        ...
        m_addressFamily(AF_INET), m_connected(false), m_socket(-1), m_port(port)
    {
        ...
        else
        {
            ...
            if (GetASockAddrFromHostName(host, (void*)sockAddrBuffer, addrLength, m_addressFamily))
            {
                ...
                m_hostIP = std::string(hostBuffer);
            }
            else
            {
                std::cerr << ALLOC_TAG << "Can't retrieve a valid ip address based on provided host: " << host << std::endl;
            }
        }
        CreateSocket(m_addressFamily, sendBufSize, receiveBufSize, nonBlocking);
    }
    ...
    void SimpleUDP::CreateSocket(int addressFamily, size_t sendBufSize, size_t receiveBufSize, bool nonBlocking)
    {
        ...
        if (sendBufSize)
        {
            ...
            if (ret)
            {
                std::cerr << ALLOC_TAG << "Failed to set UDP send buffer size to " << sendBufSize << " for socket " << sock << " error code: " << WSAGetLastError() << std::endl;
            }
            assert(ret == 0);
        }
        ...
        if (receiveBufSize)
        {
            ...
            if (ret)
            {
                std::cerr << ALLOC_TAG << "Failed to set UDP receive buffer size to " << receiveBufSize << " for socket " << sock << " error code: " << WSAGetLastError() << std::endl;
            }
            assert(ret == 0);
        }
